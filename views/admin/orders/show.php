    ...
    <div class="card-body">
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
            <div>
                ...
                <p><strong>用户：</strong><?= htmlspecialchars($order['user_name'] ?? '-') ?></p>
            </div>
            <div>
                <p>
                    <strong>状态：</strong>
        ...
        <div style="margin-top: 1.5rem;">
            ...
            <?php if ($order['status'] == 1): ?>
                <form action="/admin/orders/<?= $order['id'] ?>/ship" method="POST" style="display: inline;">
                    <button type="submit" class="btn btn-primary">确认发货</button>
                </form>
            <?php endif; ?>
            
            <?php if ($order['status'] == 2): ?>
                <form action="/admin/orders/<?= $order['id'] ?>/complete" method="POST" style="display: inline;">
                    <button type="submit" class="btn btn-success">完成订单</button>
                </form>
            <?php endif; ?>
        </div>
    </div>
